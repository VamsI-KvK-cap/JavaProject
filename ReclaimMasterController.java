...
public class ReclaimMasterController {
    ...
    public ResponseEntity<?> readReclaimMasterData() {
        try {
            reclaimService.initializeOptions();
            if (reclaimService.checkForAbortion()) {
                return ResponseEntity.status(400).body("Abortion condition met: Please check the input data.");
            }
            return ResponseEntity.ok(reclaimService.processReclaimData());
