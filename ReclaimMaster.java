...
public class ReclaimMaster {
    ...
    private List<ReclaimMaster> reclaimData;

    public ReclaimMaster(String reclaimId, String claimantName, String claimDate, double claimAmount) {
        initializeReclaim(reclaimId, claimantName, claimDate, claimAmount);
        this.reclaimData = new ArrayList<>();
    }
    ...
    public void readReclaimData(ReclaimMaster reclaim) {
        reclaimData.add(reclaim);
    }

    public void processReclaimData() {
        for (ReclaimMaster reclaim : reclaimData) {
            if ("Pending".equals(reclaim.getStatus())) {
                // Processing logic can be added here
                reclaim.approveReclaim();
            }
        }
    }
